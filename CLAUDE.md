...
Layout:
- `native/` — **the app.** `main.cpp` dispatches GUI vs CLI; `core.{h,cpp}` is UI-free logic (enumerate, guard, RNG, planner, engine, reformat); `cli.cpp` and `gui.cpp` are thin front-ends. `build.ps1` compiles it; `app.manifest` + `resource.rc` embed the admin manifest + common-controls v6. `test_plan.cpp` is a console unit test for the planner.
- `QuickWiper/`, `QuickWiper.Tests/` — the original C# implementation (same design, same behavior) plus xUnit tests for the guard/planner/RNG. **Superseded** by `native/` but kept as reference and for its unit tests. (The C# build has Full+Quick only; Quick 2 was added to the native build, the deliverable.)
- `test/plan-test.ps1` — compiles + runs `native/test_plan.cpp`: verifies the pass planner for all three modes (Full/Quick/Quick2) — chunk sizes, bit-reversed spread, and that every mode covers the whole device (so it converges to a full wipe). **No device, no elevation.**
- `test/vhd-test.ps1 [-ExePath ...]` — elevated integration test: creates a throwaway VHD, wipes it with the real exe (Quick2/Quick/Full), verifies planted markers are destroyed. Never touches a pre-existing disk. `test/effectiveness-test.ps1 [-Disk N]` — elevated, wipes a REAL USB disk to compare Quick vs Quick2 (largest intact region left after a time-boxed cancel). `test/screenshot.ps1 [-ExePath ...]` captures the GUI.

Verified: planner unit test 47/47 (all modes, incl. Quick 2's 8 MB bit-reversed full-coverage plan). Throughput of the Quick 2 chunk validated on a real Lexar USB drive (8 MB is the blocks-per-second optimum; see the Quick 2 note under Product). Earlier baseline (native build, 9/9 integration checks; C# build also 9/9 + 20 unit tests): system disk rejected (exit 2, even with `--allow-virtual`), wipe modes run and destroy secret markers, GUI renders correctly. The wipe **engine** (`RunWipe`) is unchanged by Quick 2 — only the plan differs — so the prior marker-destruction result carries over; re-run `vhd-test.ps1` elevated for an end-to-end Quick 2 check. Reformat (diskpart/VDS) couldn't be verified on the dev machine due to a local VDS timeout — code is correct; verify on healthy hardware.
...
Three wipe modes (all in `Plan`; all write high-entropy random data and converge to a full overwrite if finished — they only differ in *ordering*, which is what governs an early cancel):
- **Full** — thorough sequential random overwrite of the whole device, first sector to last.
- **Quick** — an *anytime* wipe: at every instant of cancel, maximize the data already unrecoverable. Two mechanisms: **kill the partition map** (front partition area + the backup GPT at the tail) first, then a **bit-reversed spread fill** — overwrite the whole device in 64 MB chunks visited in bit-reversed order, so finished chunks stay spread uniformly across the device at every cancel point (no front/tail blind spot) while each chunk is one large sequential write (fast on USB). Cancel anytime; if left running it overwrites every chunk, converging to Full.
- **Quick 2** (default/recommended) — same map-kill + bit-reversed spread fill, but with a **much smaller chunk** (`Quick2Chunk`, 8 MB) instead of 64 MB. The bit-reversed order already gives multi-resolution coverage for free; the right chunk is the one that maximizes *blocks written per second* (R/B), because the largest contiguous intact region left at any cancel point is ∝ chunk÷(throughput·time). 8 MB is that sweet spot on USB flash: it's the smallest chunk still written at near-sequential speed — below it, writes fall under the flash erase-block size and collapse (measured on a Lexar USB drive: 8 MB ≈ 8 MB/s but 4 MB ≈ 1 MB/s, ~20× slower, so 4 MB is actually *worse* than 64 MB on the R/B metric). At 8 MB the device is covered ~3× more finely than Quick per unit time, so the largest intact region at any cancel point is ~3× smaller. Trade-off: a full Quick 2 run is ~2–3× slower to *complete* than Quick — fine, since Quick 2 is for cancelling early; use Quick/Full when letting it finish. `AppendSpreadFill` is shared by both Quick and Quick 2; only the chunk size differs.
